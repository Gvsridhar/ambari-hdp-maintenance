#!/bin/bash
#Author - Venkata Sridhar Gangavarapu
####################################################

#Globals

source /data/ambari_maint/ambari.props
...
service_action()
{
	...
	echo -e "\nPutting all the services in $1 state!"
	 curl -s -H 'X-Requested-By:ambari' -u $AMBARI_ADMIN_USER:$AMBARI_ADMIN_PASSWORD -i -X PUT -d '{"RequestInfo": {"context" :"'"Putting All Services in $1 state"'"}, "ServiceInfo": {"state" : "'"$1"'"}}' $URL/api/v1/clusters/$CLUSTER_NAME/services > /tmp/result
	request_id=`cat /tmp/result|grep id|awk '{print $3}'|cut -d',' -f1`
	progressPercent=` curl -s --user $AMBARI_ADMIN_USER:$AMBARI_ADMIN_PASSWORD -H 'X-Requested-By:SupportLab' -X GET $URL/api/v1/clusters/$CLUSTER_NAME/requests/$request_id | grep progress_percent | awk '{print $3}' | cut -d . -f 1`
...
{
	 curl -s -H "X-Requested-By:ambari" -u $AMBARI_ADMIN_USER:$AMBARI_ADMIN_PASSWORD -i -X GET $URL/api/v1/clusters/$CLUSTER_NAME/services|grep href|cut -d':' -f2-|rev|cut -d'/' -f1|rev|cut -d'"' -f1|grep ^[A-Z] > /tmp/list_all_services
	 curl -s -H "X-Requested-By:ambari" -u $AMBARI_ADMIN_USER:$AMBARI_ADMIN_PASSWORD -i -X GET $URL/api/v1/hosts|grep host_name|cut -d':' -f2|cut -d'"' -f2|uniq > /tmp/list_all_hosts
		for host in `cat /tmp/list_all_hosts`;do echo -e "\n$(tput setaf 2)$host$(tput sgr 0)\n\n";  curl -s -H "X-Requested-By:ambari" -u $AMBARI_ADMIN_USER:$AMBARI_ADMIN_PASSWORD -i -X GET $URL/api/v1/clusters/$CLUSTER_NAME/hosts/$host/host_components|grep component_name|cut -d',' -f1|tr ':' '|';done > /tmp/list_host_components 
	if [ "$1" == "show" ]
	then
...
individual_service_action()
{
	#$1 - start/stop $2 - service name
	 curl -s -u $AMBARI_ADMIN_USER:$AMBARI_ADMIN_PASSWORD -i -H 'X-Requested-By: ambari' -X PUT -d '{"RequestInfo": {"context" :"'"Putting $2 in $1 state"'"}, "Body": {"ServiceInfo": {"state": "'"$1"'"}}}' $URL/api/v1/clusters/$CLUSTER_NAME/services/$2 > /tmp/result
        request_id=`cat /tmp/result|grep id|awk '{print $3}'|cut -d',' -f1`
        progressPercent=` curl -s --user $AMBARI_ADMIN_USER:$AMBARI_ADMIN_PASSWORD -H 'X-Requested-By:SupportLab' -X GET $URL/api/v1/clusters/$CLUSTER_NAME/requests/$request_id | grep progress_percent | awk '{print $3}' | cut -d . -f 1`
        echo " Progress: $progressPercent"
        while [[ `echo $progressPercent | grep -v 100` ]]; do
...
host_component_action()
{
	#$1-star/stop $2-service component name $3-hostname
	 curl -u $AMBARI_ADMIN_USER:$AMBARI_ADMIN_PASSWORD -i -H 'X-Requested-By: ambari' -X PUT -d '{"RequestInfo": {"context" :"'"Putting $2 in $1 state"'"}, "HostRoles": {"state": "'"$1"'"}}' $URL/api/v1/clusters/$CLUSTER_NAME/hosts/$3/host_components/$2
}
...
add_remove_host_component()
{
	...
	if [ "$1" == add ]
	then
		 curl -u $AMBARI_ADMIN_USER:$AMBARI_ADMIN_PASSWORD -i -H 'X-Requested-By: ambari' -X POST -d '{"host_components" : [{"HostRoles":{"component_name":"'"$2"'"}}] }' $URL/api/v1/clusters/$CLUSTER_NAME/hosts?Hosts/host_name=$3
	...
	then
		 curl -u $AMBARI_ADMIN_USER:$AMBARI_ADMIN_PASSWORD -H "X-Requested-By: ambari" -X DELETE $URL/api/v1/clusters/$CLUSTER_NAME/hosts/$3/host_components/$2
	fi
}
...
{
	echo " curl -u $AMBARI_ADMIN_USER:$AMBARI_ADMIN_PASSWORD $URL/api/v1/clusters/$CLUSTER_NAME/host_components?HostRoles/stale_configs=true&fields=HostRoles/service_name,HostRoles/host_name&minimal_response=false"> /tmp/ curl_ambari.sh
